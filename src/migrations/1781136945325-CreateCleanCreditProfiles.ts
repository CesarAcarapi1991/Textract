...
export class CreateCleanCreditProfiles1780000000013
  implements MigrationInterface
{
  name = 'CreateCleanCreditProfiles1780000000013';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TABLE ${q}."clean_credit_profiles_ca" (
        ...
        CONSTRAINT "FK_clean_credit_profiles_application_ca"
          FOREIGN KEY ("application_id") REFERENCES ${q}."credit_applications"("id")
      )
    `);

    await queryRunner.query(`
      CREATE TABLE ${q}."glue_job_runs_ca" (
        "id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
        ...
        "created_at" timestamptz NOT NULL DEFAULT now(),
        "updated_at" timestamptz NOT NULL DEFAULT now(),
        CONSTRAINT "FK_glue_job_runs_application_ca"
          FOREIGN KEY ("application_id") REFERENCES ${q}."credit_applications"("id")
      )
    `);
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    const schema = process.env.DATABASE_SCHEMA ?? 'public';
    const q = `"${schema}"`;
    await queryRunner.query(`DROP TABLE IF EXISTS ${q}."glue_job_runs_ca"`);
    await queryRunner.query(`DROP TABLE IF EXISTS ${q}."clean_credit_profiles_ca"`);
  }
}