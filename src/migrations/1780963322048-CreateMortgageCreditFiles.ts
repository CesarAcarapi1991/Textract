...
export class CreateMortgageCreditFiles1780963322048
  implements MigrationInterface
{
  name = 'CreateMortgageCreditFiles1780963322048';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TABLE ${q}."document_types_ca" (
    ...
    await queryRunner.query(`
      CREATE TABLE ${q}."credit_applications_ca" (
        "id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    await queryRunner.query(`
      CREATE TABLE ${q}."application_documents_ca" (
        "id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
        ...
        CONSTRAINT "FK_application_documents_application_ca"
          FOREIGN KEY ("application_id") REFERENCES ${q}."credit_applications"("id"),
        CONSTRAINT "FK_application_documents_document_type_ca"
          FOREIGN KEY ("document_type_code") REFERENCES ${q}."document_types"("code")
      )
    `);

    await queryRunner.query(`
      CREATE TABLE ${q}."textract_results_ca" (
        "id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
        "application_id" uuid NOT NULL,
        ...
        "created_at" timestamptz NOT NULL DEFAULT now(),
        CONSTRAINT "FK_textract_results_application_ca"
          FOREIGN KEY ("application_id") REFERENCES ${q}."credit_applications"("id"),
        CONSTRAINT "FK_textract_results_document_ca"
          FOREIGN KEY ("document_id") REFERENCES ${q}."application_documents"("id")
      )
    `);

    await queryRunner.query(`
      CREATE TABLE ${q}."textract_query_answers_ca" (
        "id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
        "application_id" uuid NOT NULL,
        "document_id" uuid NOT NULL,
        "document_type_code" text NOT NULL,
        ...
        "created_at" timestamptz NOT NULL DEFAULT now(),
        CONSTRAINT "FK_query_answers_application_ca"
          FOREIGN KEY ("application_id") REFERENCES ${q}."credit_applications"("id"),
        CONSTRAINT "FK_query_answers_document_ca"
          FOREIGN KEY ("document_id") REFERENCES ${q}."application_documents"("id")
      )
    `);

    await queryRunner.query(`
      CREATE TABLE ${q}."application_extracted_data_ca" (
        "id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
        ...
        "created_at" timestamptz NOT NULL DEFAULT now(),
        "updated_at" timestamptz NOT NULL DEFAULT now(),
        CONSTRAINT "FK_application_extracted_data_application_ca"
          FOREIGN KEY ("application_id") REFERENCES ${q}."credit_applications"("id")
      )
    `);

    await queryRunner.query(`
      INSERT INTO ${q}."document_types_ca" ("code", "name", "category", "queries")
      VALUES
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    const schema = process.env.DATABASE_SCHEMA ?? 'public';
    const q = `"${schema}"`;
    await queryRunner.query(`DROP TABLE IF EXISTS ${q}."application_extracted_data_ca"`);
    await queryRunner.query(`DROP TABLE IF EXISTS ${q}."textract_query_answers_ca"`);
    await queryRunner.query(`DROP TABLE IF EXISTS ${q}."textract_results_ca"`);
    await queryRunner.query(`DROP TABLE IF EXISTS ${q}."application_documents_ca"`);
    await queryRunner.query(`DROP TABLE IF EXISTS ${q}."credit_applications_ca"`);
    await queryRunner.query(`DROP TABLE IF EXISTS ${q}."document_types_ca"`);
  }
}